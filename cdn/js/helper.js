...
/**
 * MutationObserver does not descend into shadow roots, AND it does not fire
 * when an element gets upgraded and calls attachShadow. Nested custom elements
 * like <mfe-header> inside <mfe-home>'s shadow root attach their own shadow
 * later — after their JS bundle loads — and the observer never sees it.
 *
 * Cover that case by patching attachShadow itself: every shadow root that
 * exists in the page passes through here at construction time, so we can
 * inject boundary styles immediately and observe the new root.
 *
 * `observedRoots` dedupes so we don't attach multiple observers per root.
 */
...
Element.prototype.attachShadow = function (init) {
  ...
  // Defer so the caller can populate the shadow root before we walk it.
  queueMicrotask(() => attachToShadow(shadowRoot));
  return shadowRoot;
};
...
/**
 * Attach the boundary stylesheet to a shadow root and start observing it
 * so any nested shadow roots get the same treatment.
 * @param {ShadowRoot} shadowRoot
 */
function attachToShadow(shadowRoot) {
  if (!shadowRoot || observedRoots.has(shadowRoot)) return;
  observedRoots.add(shadowRoot);
  copyStylesToShadow(shadowRoot);
  // Walk current descendants for already-attached shadow roots.
  shadowRoot.querySelectorAll("*").forEach((el) => {
    if (el.shadowRoot) attachToShadow(el.shadowRoot);
  });
  // Watch future additions inside this shadow root.
  observeRoot(shadowRoot);
}

/**
 * Walk an added subtree looking for shadow roots to wire up.
 * @param {Node} node
 */
function processNode(node) {
  if (node.shadowRoot) attachToShadow(node.shadowRoot);
...
/**
 * Observe a root (Document or ShadowRoot) for added subtrees. Each new
 * shadow root encountered recursively becomes its own observation target.
